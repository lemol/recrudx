...
export function editForm(options, view = 'edit') {
  const { schema, type, namespace, apiActions, componentActions } = options
  const fields = options.schema.fields.map(s => s.name)

  const mapStateToProps = (state) => Object.assign({
    initialValues: state[namespace][type][view].item,
    sources: state[namespace][type][view].sources,
    params: state[namespace][type][view].params
  })
  ...
  const mergeProps = (stateProps, dispatchProps) => Object.assign({
    ...
    fetchSource: dispatchProps.fetchSource.bind(null, view, schema),
    updateParams: dispatchProps.updateParams.bind(null, view, schema)
  })


  const form = `${namespace}_${type}_${view}_form`
